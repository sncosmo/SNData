...
class Balland09(DataRelease):
    ...
    def __init__(self):
        ...
        self.spectra_dir = self.data_dir / 'spectra'  # DR1 spectra
        self.table_dir = self.data_dir / 'tables'  # DR3 paper tables

        # Define urls for remote data
        self.phase_spectra_url = 'http://supernovae.in2p3.fr/~balland/VltRelease/PHASE_spec_Balland09.tar.gz'
        self.snonly_spectra_url = 'http://supernovae.in2p3.fr/~balland/VltRelease/snonly_spec_Balland09.tar.gz'
        self.table_url = 'http://cdsarc.u-strasbg.fr/viz-bin/nph-Cat/tar.gz?J/A+A/507/85'
    ...
    def _get_available_ids(self):
        ...
        files = self.spectra_dir.glob('*.dat')
    ...
    def _get_balland_meta(self, obj_id):
        ...
        ra_hourangle = (
            object_data['RAh'], object_data['RAm'], object_data['RAs'])
    ...
    def _get_data_for_id(self, obj_id: str, format_table: bool = True):
        ...
        tables = []
        for fpath in self.spectra_dir.glob(
                f'*_{obj_id}_*_Balland_etal_09.dat'):
            ...
            data_table['phase'] = float(
                data_table.meta['comments'][7].split()[-1])
    ...
    def download_module_data(self, force: bool = False):
        ...
        if (force or not self.table_dir.exists()) and utils.check_url(
                self.table_url):
        ...
        fix_balland09_cds_readme(self.table_dir / 'ReadMe')

        # Download spectra
        if (force or not self.spectra_dir.exists()):
            spec_urls = self.phase_spectra_url, self.snonly_spectra_url
